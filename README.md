...
You can call the new FastAPI endpoint `POST /score` to get a basic ATS score.

Example (resume text in form):
```bash
curl -X POST "http://localhost:8000/score" \
  -F "job_description=@jd.txt;type=text/plain" \
  -F "resume_text=$(cat resume.txt)"
```
...
Response:
```json
{ "score": 72, "matched_keywords": [...], "missing_keywords": [...], "total_keywords": 25 }
```
